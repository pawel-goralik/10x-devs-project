...
| 6 | *(abuse lens)* The magic-link request endpoint can be used to flood an arbitrary email address with authentication emails, and/or a group's non-expiring, non-revocable invite token is guessed or enumerated by a non-member | Medium | Low-Medium | PRD Access Control ("closed-circle… trusted composition… risk accepted"); archived slice `form-and-manage-a-group/plan.md` hard constraint (no invite expiry/revocation); mandatory abuse/security lens (resource abuse + authorization/access classes) |
...
|---|---|---|---|---|---|---|
| 1 | Critical-path authorization & immutability coverage | Bootstrap the test runner and defend the two highest-impact core guarantees: cross-group visibility and the 24h lock boundary | #1, #2 | unit + integration | change opened | `context/changes/testing-critical-path-coverage/` |
...
- TBD — see §3 Phase 1 (bootstraps Vitest; no test runner exists yet).

### 6.2 Adding an integration test for an authorization/visibility boundary

- TBD — see §3 Phase 1 (cross-group visibility and 24h-lock-boundary pattern, Risks #1–#2).
