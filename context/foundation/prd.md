...
The same archetype shows up in two adjacent framings, both in scope for the MVP — these are not two personas, but the same person seen from either side of an invitation:
...
Authenticated capabilities (the role-to-capability matrix):
